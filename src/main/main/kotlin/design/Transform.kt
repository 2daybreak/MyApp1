...
class Transform {
    ...
    fun getWorldMatrix(offset: Vector3f, rotation: Vector3f): Matrix4f {
    ...
    fun getWorldMatrix(offset: Vector3f, quaternion: Quaternionf, scale: Float): Matrix4f {
        val worldMatrix = Matrix4f().identity()
                .translate(offset)
                .rotate(quaternion)
                .scale(scale)
    ...
    fun getWorldMatrix(camera: Camera, quaternion: Quaternionf, scale: Float): Matrix4f {
        val p = camera.position
        val f = camera.front
        val u = camera.up
        val worldMatrix = Matrix4f().identity()
